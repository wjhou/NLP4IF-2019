...
from pytorch_transformers import (WEIGHTS_NAME, BertConfig,
                                  ...
                                  XLNetTokenizer, OpenAIGPTConfig, 
                                  OpenAIGPTDoubleHeadsModel, OpenAIGPTTokenizer, OpenAIGPTDoubleHeadsModel, 
                                  RobertaConfig, RobertaForSequenceClassification, RobertaTokenizer)

from pytorch_transformers import AdamW, WarmupLinearSchedule

from data_utils import (compute_metrics, convert_examples_to_features,
...
def evaluate(args, model, tokenizer, prefix="", is_eval=True):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        eval_dataset, eval_examples = load_and_cache_examples(args, eval_task, tokenizer, evaluate=is_eval, test=(not is_eval))
        ...
        output_pred_file = os.path.join(eval_output_dir, "preds_%s.txt"%end_date)
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, test=False):
    ...
    cached_features_file = os.path.join(args.data_dir, 'cached_{}_{}_{}_{}'.format(
        ('dev' if evaluate else 'test') if (evaluate or test) else 'train',
        list(filter(None, args.model_name_or_path.split('/'))).pop(),
        str(args.max_seq_length),
        str(task)))
    if (evaluate or test) and os.path.exists(cached_features_file):
        logger.info("Loading features from cached file %s", cached_features_file)
        features = torch.load(cached_features_file)
    else:
        logger.info("Creating features from dataset file at %s", args.data_dir)
        label_list = processor.get_labels()
        examples = (processor.get_test_examples(args.data_dir) if test else processor.get_dev_examples(args.data_dir)) if (evaluate or test) else processor.get_train_examples(args.data_dir)
...
def main():
    ...
    # Training
    if args.do_train:
        sample = "sample" in args.task_name.lower()
#         train_dataset = load_and_cache_examples(args, args.task_name, tokenizer, evaluate=False)
#         global_step, tr_loss = train(args, train_dataset, model, tokenizer)
        global_step, tr_loss = train(args, load_and_cache_examples, model, tokenizer, sample)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result = evaluate(args, model, tokenizer, prefix=global_step, is_eval=True)
    ...
    if args.do_test and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model = model_class.from_pretrained(checkpoint)
            model.to(args.device)
            result = evaluate(args, model, tokenizer, prefix=global_step, is_eval=False)
            result = dict((k + '_{}'.format(global_step), v) for k, v in result.items())
